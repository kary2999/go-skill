...
(function () {
  ...
  // ---------- 测试 Skill（v1.7.10：单一 clawnova 配置）----------
  let evalCasesCache = null;
  let evalProviderMeta = null; // 唯一 provider（clawnova）
  let evalConfig = null;       // 持久化配置（含 has_key / key_masked / model 等）

  function initEval() {
    Promise.all([
      fetch('/api/eval/cases').then((r) => r.json()),
      fetch('/api/eval/config').then((r) => r.json()),
    ]).then(([cases, cfg]) => {
      evalCasesCache = cases.cases || [];
      evalProviderMeta = (cases.providers || [])[0]; // 现在只有 clawnova
      evalConfig = cfg;
      document.getElementById('evalCount').textContent = evalCasesCache.length;
      document.getElementById('evalTotalCases').textContent = evalCasesCache.length;
      renderProviderConfig();
    });
  }

  function renderProviderConfig() {
    if (!evalProviderMeta || !evalConfig) return;
    document.getElementById('evalProviderName').textContent = evalProviderMeta.name;
    document.getElementById('evalApiBase').value = evalConfig.api_base || 'https://clawnova.ai/api/v1';
    document.getElementById('evalKey').value = ''; // 永远空，看星号化预览
    document.getElementById('evalKey').placeholder = evalProviderMeta.key_placeholder || '';

    // v1.7.12: 显示 key 健康状态
    const maskedEl = document.getElementById('evalKeyMasked');
    if (evalConfig.key_health === 'bad_is_url') {
      maskedEl.innerHTML = '<span style="color:#ff6b6b;">⚠️ 已存的 key 看起来是个 URL（' +
        evalConfig.key_masked + '），明显错了。点下面「🗑 清除已存 Key」回落默认 key。</span>';
    } else if (evalConfig.key_health === 'bad_too_short') {
      maskedEl.innerHTML = '<span style="color:#ff6b6b;">⚠️ 已存 key 太短（' +
        evalConfig.key_masked + '），不像真 key。请清除重粘。</span>';
    } else if (evalConfig.has_key) {
      maskedEl.textContent = evalConfig.key_masked;
    } else {
      maskedEl.textContent = '未配置';
    }

    const sel = document.getElementById('evalModel');
    sel.innerHTML = '';
    (evalProviderMeta.models || []).forEach((m) => {
      const opt = document.createElement('option');
      opt.value = m; opt.textContent = m;
      if (m === evalConfig.model) opt.selected = true;
      sel.appendChild(opt);
    });
  }
  initEval();
  ...
  // 保存配置
  document.getElementById('evalConfigSaveBtn').addEventListener('click', () => {
    const apiBase = document.getElementById('evalApiBase').value.trim();
    const key = document.getElementById('evalKey').value.trim();
    const model = document.getElementById('evalModel').value;
    const temp = parseFloat(document.getElementById('evalTemp').value) || 0;
    const body = { api_base: apiBase, model, temperature: temp };
    if (key) body.api_key = key; // 空 = 不变（保留旧 key）

    const btn = document.getElementById('evalConfigSaveBtn');
    ...
    fetch('/api/eval/config', {
      ...
      if (status === 400 && res.error) {
        // 防呆校验拦下来了（URL 当 key / key 太短）
        showLog('evalConfigLog', '✗ ' + res.error, 'err');
        return;
      }
      if (res.ok) {
        showLog('evalConfigLog', '✓ 已保存。Key: ' + res.key_masked, 'ok');
        document.getElementById('evalKey').value = ''; // 清空
        // 刷新配置
        fetch('/api/eval/config').then((r) => r.json()).then((cfg) => {
          evalConfig = cfg; renderProviderConfig();
        });
      } else {
        showLog('evalConfigLog', '✗ 保存失败：' + (res.error || ''), 'err');
      }
    });
  });

  document.getElementById('evalConfigClearBtn').addEventListener('click', () => {
    if (!confirm('确认清除已保存的 Key？API Base 和模型选择会保留。')) return;
    fetch('/api/eval/config/key', { method: 'DELETE' })
      .then((r) => r.json()).then(() => {
        showLog('evalConfigLog', '✓ 已清除 Key', 'ok');
        fetch('/api/eval/config').then((r) => r.json()).then((cfg) => {
          evalConfig = cfg; renderProviderConfig();
        });
      });
  ...
  // 测试连接
  document.getElementById('evalTestConnBtn').addEventListener('click', () => {
    const btn = document.getElementById('evalTestConnBtn');
    btn.disabled = true; btn.textContent = '测试中（最多 60s）…';
    showLog('evalConfigLog', '→ 用当前配置发一个最小 OpenAI 兼容请求...', '');
    fetch('/api/eval/test-connection', { method: 'POST' })
      .then((r) => r.json()).then((res) => {
        btn.disabled = false; btn.textContent = '🔌 测试连接';
        if (res.ok) {
          showLog('evalConfigLog',
            '✓ 连接成功 · 模型 ' + res.model + '\n' +
            '  Base: ' + res.base + '\n' +
            '  in/out tokens: ' + (res.input_tokens || 0) + ' / ' + (res.output_tokens || 0) + '\n' +
            '  reply: ' + (res.reply || '').substring(0, 200),
            'ok');
        } else {
          showLog('evalConfigLog', '✗ 失败：' + res.error, 'err');
        }
      });
  });

  // v1.7.13: 改成前端串行跑（一次一个 case_id）
  // 原因：WKWebView fetch 默认 60s 超时，20 条串行 ~3-4 分钟必触发。
  //      改成前端按条调用，每次只等一条（10-30s），同时实时显示进度。
  async function doEvalRun(caseIds, btn, btnText, label) {
    if (!evalConfig || !evalConfig.has_key) {
      showLog('evalLog', '✗ 还没保存 API Key —— 先在上面填 key 点「💾 保存配置」', 'err');
      return;
    }
    const temp = parseFloat(document.getElementById('evalTemp').value) || 0;
    ...
    if (caseIds && caseIds.length) {
      ...
      // 跑全部：从 evalCasesCache 提取所有 ID
      runIDs = (evalCasesCache || []).map((c) => c.id);
    }
    if (runIDs.length === 0) {
      showLog('evalLog', '✗ 没有可跑的 case', 'err');
      return;
    }

    btn.disabled = true;
    document.getElementById('evalReport').style.display = 'none';
    ...
    for (let i = 0; i < runIDs.length; i++) {
      const caseId = runIDs[i];
      btn.textContent = `跑 ${i + 1}/${runIDs.length}…`;
      const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
      showLog('evalLog',
        `[${i + 1}/${runIDs.length}] ${caseId} 进行中… 已用时 ${elapsed}s · 通过 ${passed}/${i}`,
        '');
      ...
      try {
        ...
        if (res.error) {
          // 服务端级错误（如缺 key）
          showLog('evalLog', `✗ ${res.error}`, 'err');
          break;
        }
        const cases = res.cases || [];
        if (cases.length > 0) {
          allCases.push(cases[0]);
          if (cases[0].pass) passed++;
          if (cases[0].error) {
            consecutiveErrors++;
            // 连续 3 条都报错 = API 真出问题，停早些
            if (consecutiveErrors >= 3) {
              showLog('evalLog',
                `✗ 连续 3 条 case 报错，停止后续。最后错误：\n${cases[0].error}\n\n` +
                `已跑 ${i + 1}/${runIDs.length}，通过 ${passed}。`, 'err');
              break;
            }
          } else {
            consecutiveErrors = 0;
    ...
    if (allCases.length === runIDs.length) {
      showLog('evalLog', `✓ 完成，总耗时 ${totalSec}s · ${passed}/${allCases.length} 通过`, 'ok');
    } else if (allCases.length > 0) {
      showLog('evalLog',
        `⚠ 部分完成（${allCases.length}/${runIDs.length}），总耗时 ${totalSec}s · 通过 ${passed}`, 'err');
    }

    // 用累加结果渲染报告（renderEvalReport 期望 EvalResult 结构）
    renderEvalReport({
      ok: true,
      total: runIDs.length,
      passed: passed,
  ...
  document.getElementById('evalRunBtn').addEventListener('click', (e) => {
    doEvalRun(null, e.currentTarget, '🚀 运行全部 API 测试', '全部 20 条');
  });
  document.getElementById('evalPingBtn').addEventListener('click', (e) => {
    doEvalRun(['iron-01-hardcoded-secret'], e.currentTarget, '🔌 先跑 1 条测连通', '连通测试 1 条');
  });

  // 🔍 SKILL 结构自检 —— 零 API，毫秒出结果
  document.getElementById('evalLintBtn').addEventListener('click', () => {
    showLog('evalLog', '正在静态扫描 SKILL.md + references...', '');
    fetch('/api/eval/lint-skill', { method: 'POST' })
      .then((r) => r.json())
      .then((d) => {
        const checks = d.checks || [];
        let html = `✓ 结构自检完成（${d.passed}/${d.total} 通过）\n\n`;
        checks.forEach((c) => {
          html += (c.pass ? '  ✓ ' : '  ✗ ') + c.name + '\n';
          html += '     ' + c.detail + '\n';
        });
        const cls = d.passed === d.total ? 'ok' : 'err';
        showLog('evalLog', html, cls);
      })
      .catch((e) => showLog('evalLog', '✗ ' + e, 'err'));
  });

  // 📋 导出测试 prompt —— 零 API，生成 md 文件
  document.getElementById('evalExportBtn').addEventListener('click', () => {
    showLog('evalLog', '生成测试 prompt 文件...', '');
    fetch('/api/eval/export-prompts', { method: 'POST' })
      .then((r) => r.json())
      .then((res) => {
        if (!res.ok) {
          showLog('evalLog', '✗ 生成失败：' + (res.error || '未知'), 'err');
          return;
        }
        ...
        el.classList.add('ok', 'show');
        el.innerHTML =
          '✓ 已生成（' + res.cases + ' 个用例，' + (res.size / 1024).toFixed(1) + ' KB）\n\n' +
          '文件：<code>' + escapeHTML(res.path) + '</code>\n\n' +
          '<button class="btn btn-ghost btn-sm" id="revealPromptBtn">🔍 在 Finder 中显示</button>\n\n' +
          '<strong>使用方法</strong>：\n' +
          '  1. 打开生成的 md 文件\n' +
          '  2. 对每个用例，把 Go 代码段 + 指令粘到 Cursor / Claude Code\n' +
          '  3. 人工判断 AI 回复是否按三段式指出违规\n' +
          '  4. 勾对应的 checkbox 统计通过率\n\n' +
          '这种测试用的是你日常工作的 Cursor / Claude Code，skill 已生效环境，结果最真实。';
        document.getElementById('revealPromptBtn').addEventListener('click', () => {
          fetch('/api/reveal', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ path: res.path }),
  ...
  function renderEvalReport(res) {
    const el = document.getElementById('evalReport');
    el.style.display = 'block';
    const rate = res.total ? Math.round((res.passed / res.total) * 100) : 0;
    ...
    (res.cases || []).forEach((c) => {
      ...
      totalCacheCreate += c.cache_create_tokens || 0;
    });

    document.getElementById('evalSummary').innerHTML = `
      <div class="eval-stat ${res.passed === res.total ? 'ok' : (res.passed === 0 ? 'err' : '')}">
        <div class="eval-stat-label">通过率</div>
        <div class="eval-stat-value">${res.passed} / ${res.total} · ${rate}%</div>
      </div>
      <div class="eval-stat">
        <div class="eval-stat-label">总耗时</div>
        <div class="eval-stat-value">${(res.total_time_ms / 1000).toFixed(1)} s</div>
      </div>
      <div class="eval-stat">
        <div class="eval-stat-label">Cache 命中</div>
        <div class="eval-stat-value">${totalCacheRead.toLocaleString()} tok</div>
      </div>
      <div class="eval-stat">
        <div class="eval-stat-label">Output</div>
        <div class="eval-stat-value">${totalOutput.toLocaleString()} tok</div>
      </div>
      <div class="eval-progress">
        <div class="eval-progress-bar" style="width: ${rate}%"></div>
      </div>
    `;

    const casesEl = document.getElementById('evalCases');
    casesEl.innerHTML = '';
    (res.cases || []).forEach((c, idx) => {
      const box = document.createElement('div');
      box.className = 'eval-case ' + (c.pass ? 'pass' : 'fail');
      const icon = c.pass ? '✓' : '✗';
      const matched = (c.matched_keywords || []).map((k) => `<span class="kw-chip hit">✓ ${escapeHTML(k)}</span>`).join('');
      const missed = (c.missed_keywords || []).map((k) => `<span class="kw-chip miss">○ ${escapeHTML(k)}</span>`).join('');
      // 错误直接显示在卡片头部（不等展开）
      const inlineErr = c.error ? `<div class="eval-case-err">🚨 ${escapeHTML(c.error)}</div>` : '';
      const metaText = c.error
        ? 'API 错误 · ' + c.latency_ms + 'ms'
        : c.hit_count + '/' + c.required + ' 关键词 · ' + c.latency_ms + 'ms';
      box.innerHTML = `
        <div class="eval-case-head" data-idx="${idx}">
          <div class="eval-case-title">
            <span class="eval-case-icon ${c.pass ? 'pass' : 'fail'}">${icon}</span>
            ${escapeHTML(c.rule)}
          </div>
          <span class="eval-case-meta">${metaText}</span>
        </div>
        ${inlineErr}
        <div class="eval-case-body">
          <h4>命中的关键词</h4>
          <div>${matched || '<span style="color:var(--text-dim)">（无）</span>'}</div>
          <h4>未命中</h4>
          <div>${missed || '<span style="color:var(--text-dim)">（无）</span>'}</div>
          <h4>AI 实际回复</h4>
          <div class="ai-resp">${escapeHTML(c.ai_response || '（空）')}</div>
        </div>
      `;
      box.querySelector('.eval-case-head').addEventListener('click', () => {
